                ...
                if (self.bytesUploaded < self.size) {
                    self.read(file, options, function(err, res) {
                        if (err) {
                            ...
                            Meteor.apply(
                                method, [self].concat(options.params || []), {
                                    wait: true
                                },
                                function(err, res) {
                                    ...
                                    if (err) {
                                        ...
                                        if (res !== undefined && typeof(res.url) !== undefined)
                                            self.url = res.url;
...
if (Meteor.isServer) {
    ...
    Npm.require('events').EventEmitter.prototype._maxListeners = 0;
    ...
    _.extend(AzureFile.prototype, {
        ...
        azureUpload: function(fileName, accountName, key, container, limit) {
            ...
            blobService.createBlockFromStream(blockId, container, fileName, stream, stream.size(), function(err, response) {
                if (err) {
                    ...
                    if (self.bytesUploaded + self.data.length >= self.size) {
                        blobService.commitBlocks(container, fileName, {
                            LatestBlocks: self.blockArray
